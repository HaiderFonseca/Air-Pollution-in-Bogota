 ...
 */
export const AVAILABLE_POLLUTANTS = [
    'PM2.5',
    'PM10',
    'NO2',
    'SO₂',
 ...
 */
export function generateMockConcentrations() {
    ...
    MOCK_SECTORS.forEach((sector) => {
        AVAILABLE_YEARS.forEach((year) => {
            AVAILABLE_POLLUTANTS.forEach((pollutant) => {
                ...
                const sectorVariation = Math.sin(sector.setuCcnct.charCodeAt(0)) * (range.max - range.min) * 0.1;
                let value = range.min + (range.max - range.min) * 0.5 + yearVariation + sectorVariation;
 ...
 */
function getUnitForPollutant(pollutant) {
    const units = {
        'PM2.5': 'µg/m³',
        'PM10': 'µg/m³',
        'NO2': 'ppb',
 ...
 */
export function getConcentrationValue(setuCcnct, pollutant, year, allConcentrations) {
    const result = allConcentrations.find((c) => c.setuCcnct === setuCcnct && c.pollutant === pollutant && c.year === year);
